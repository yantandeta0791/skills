...
Pick whichever shape best fits the question being asked, *not* whichever is easiest to wire to a terminal UI. Keep it pure: no I/O, no terminal code, no logging for control flow. The shell imports it and calls into it; nothing flows the other direction.
...
### 4. Build the smallest terminal UI that exposes the state

Build it as a **lightweight full-frame renderer** — on every tick, clear the screen and re-render the whole frame. The user should always see one stable view, not an ever-growing scrollback.
...
2. **Read one keystroke (or one line)** at a time, dispatch to a handler that applies it to state.
...
Add an entry to the project's existing task runner (`package.json` scripts, `Makefile`, `justfile`, `pyproject.toml`, or equivalent). The user should run one named command — never need to remember a path.
...
- **Don't blur the logic and the shell together.** If the reducer or state machine references prompts, logging, or terminal escape codes, it's no longer portable. Keep the shell thin over a pure module.
- **Don't ship the shell into production.** It's optimised for being driven by hand from a terminal. The logic module behind it is the bit worth keeping.
